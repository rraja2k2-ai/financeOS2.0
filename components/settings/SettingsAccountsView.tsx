...
/**
 * Correct Balance (Decision 1) — two steps in one small form: enter the actual balance,
 * then confirm the computed difference before it's saved as a real Adjustment
 * transaction. There is nothing to discard on Cancel at either step — nothing is written
 * until Confirm.
 */
function CorrectBalanceForm({
  ...
  function handleConfirm() {
    setPending(true);
    setError(null);
    correctAccountBalanceAction(account.id, newBalance)
      .then(onDone)
      .catch((e) => {
